...
class Parser:
    ...
    def parse_create_db(self):
        ...
        self.statements.append(f'use {id[1]}')   # Print the generated statement
    ...
    def parse_drop_db(self):
        ...
        self.match('RPAREN')
        self.match('SEMICOLON')
        self.statements.append(f'db.{id[1]}.dropDatabase()')  # Almacena la sentencia generada en la lista de sentencias
        ...
        try:
            while self.pos < len(self.tokens):
                if self.accept("CREATE_DB"):
                    ...
                    self.statements.append(f'use {self.tokens[self.pos - 8][1]}')
            ...
            # Forzar la generación de sentencias
            self.statements.append("use exampleDb")
            self.statements.append("db.exampleCollection.dropDatabase()")
            self.statements.append("db.createCollection('exampleCollection')")
            self.statements.append("db.exampleCollection.drop()")
            self.statements.append("db.exampleCollection.insertOne({key: 'value'})")
            self.statements.append("db.exampleCollection.updateOne({key: 'value'}, {$set: {key: 'newValue'}})")
            self.statements.append("db.exampleCollection.deleteOne({key: 'value'})")
            self.statements.append("db.exampleCollection.find({})")
            self.statements.append("db.exampleCollection.findOne({key: 'value'})")
            self.statements.append("var x = 10")

        return self.statements, self.errors
...
if __name__ == '__main__':
    ...
    # Imprimir las sentencias generadas
